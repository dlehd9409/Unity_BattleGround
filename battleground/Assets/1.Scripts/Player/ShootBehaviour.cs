...
/// 발사 키 입력받아 애니메이션 재생, 이펙트 생성, 충돌 체크 기능
/// UI 관련 크로스헤어 표시 기능
/// 발사속도 조정
/// 캐릭터 상체를 IK를 이용하여 조준 시점에 맞춰 회전
...
public class ShootBehaviour : GenericBehaviour
{
    ...
    // 짧은 총 (피스톨 등을 들었을 때 조준시 왼팔의 위치 보정)
    ...
    private void ShootWeapon(int weapon, bool firstShot = true)
    {
        ...
        else
        {
            ...
            Vector3 imprecison = Random.Range(-shootErrorRate, shootErrorRate) * behaviourController.playerCamera.forward;
            Ray ray = new Ray(behaviourController.playerCamera.position, behaviourController.playerCamera.forward + imprecison);
