...
public class AimBehaviour : GenericBehaviour
{
    ...
    void Rotating()
    {
        ...
        float minSpeed = Quaternion.Angle(myTransform.rotation, targetRotation) * aimTrunSmoothing;

        if (peekCorner)
        {
            // 조준 중일때 플레이어 상체만 살짝 기울여 준다
    ...
    public override void LocalFixedUpdate()
    {
        if (aim)
        {
            behaviourController.GetCamScript.SetTargerOffset(aimPivotOffset, aimCamOffset);
        }
    }
