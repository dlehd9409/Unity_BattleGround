...
using UnityEngine;
/// <summary>
/// 인지 타입에 따라 특정 거리로 부터 가깝진 않지만 시야는 막히지 않았으나 위험요소를 감지했거나
...
public class FocusDecision : Decision
{
    ...
    private float radius; // sence -> range
    ...
    {
        return (sence != Sence.NEAR && controller.variables.feelAlert && !controller.BlockedSight()) || Decision.CheckTargetsInRadius(controller, radius, MyHandleTargets);
    }
}
